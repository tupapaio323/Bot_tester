...
const plantillaWizard = new Scenes.WizardScene(
  ...
  // Paso 4: Recibir Cantidad y GENERAR IMAGEN DIRECTAMENTE
  async (ctx) => {
    if (!ctx.message || !ctx.message.text) {
    ...
    await ctx.reply('🎨 Generando y editando la plantilla, un momento...');
    ...
    try {
      ...
      // 4. Calculamos los puntos medios de la imagen
      const centerX = image.width / 2;
      const centerY = image.height / 2;
      const espaciado = 75; // Distancia vertical entre renglones

      // 5. Estampar los datos centrados en la imagen
      ...
      await ctx.reply('❌ Ocurrió un error al generar la plantilla. Verifica que "plantilla1.png" existe en la raíz de tu proyecto.');
    }
