...
def leer_qr_sync(path: str):
    ...
    for s in [1.0, 1.5]:
        ...
        for thr in [None, 160, 200]:
            ...
            for d in zxingcpp.read_barcodes(variant):
                seen.add(d.text)
        if seen:
            return list(seen)
    return list(seen)
...
def _cargar_fuente(*rutas, size=62):
    for r in rutas:
        try:
...
FONT = _cargar_fuente(FUENTE)
FONT_BANCOL = _cargar_fuente(FUENTE_BANCOL)
FONT_BANCOL_SANS = _cargar_fuente(FUENTE_BANCOL_SANS)
